...
def build_pattern_analysis_warning(pattern_hooks: list[str]) -> str:
    ...
    return (
        ...
        f"対象フック:\n{hook_list}\n\n"
        "【分析ツール】\n"
        ".claude/scripts/analyze-pattern-data.py を使用してパターンを分析できます:\n"
        '  python3 analyze-pattern-data.py search --pattern "検索パターン" --show-matches\n'
        '  python3 analyze-pattern-data.py analyze --pattern "分析パターン"\n'
        "  python3 analyze-pattern-data.py validate --patterns-file patterns.txt"
    )
