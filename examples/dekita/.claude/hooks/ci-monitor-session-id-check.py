#!/usr/bin/env python3
"""ci-monitor.pyのsession-idオプション指定を検出する。

Why:
    session-idが指定されないと、ci-monitor.pyはppidベースのセッション特定に
    フォールバックし、ログがClaude Codeセッションと正しく紐付かなくなる。

What:
    - ci-monitor.py呼び出しを検出
...
def main():
    ...
    try:
        ...
        # Check if this is a ci-monitor.py call
        if not re.search(r"ci-monitor\.py", command):
            # Not a ci-monitor.py call, approve
        ...
        # ci-monitor.py called without --session-id - warn
        warning = (
