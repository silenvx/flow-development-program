...
SELF_HOOK_NAME = "hook-effectiveness-evaluator"
...
def main():
    ...
    try:
        ...
        if input_data.get("stop_hook_active"):
            print_approve_and_log_skip("hook-effectiveness-evaluator", "stop_hook_active", ctx=ctx)
            return
        ...
        if not logs:
            # No logs to analyze
            log_hook_execution("hook-effectiveness-evaluator", "approve", "ログなし")
        ...
        print(f"[hook-effectiveness-evaluator] Error: {e}", file=sys.stderr)
        log_hook_execution("hook-effectiveness-evaluator", "approve", f"Error: {e}")
