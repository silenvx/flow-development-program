#!/usr/bin/env python3
"""ci_monitor.pyを使用すべきコマンドをブロックする。

Why:
    gh pr checks --watch や手動ポーリングは冗長なログでコンテキストを消費し、
    BEHIND検知や自動リベースなどの機能がない。ci_monitor.pyを使用すべき。
...
# Patterns for manual PR state check commands that should use ci_monitor.py instead.
...
def detect_manual_pr_check(command: str) -> tuple[bool, str | None]:
    ...
    Detects commands that manually check PR state instead of using ci_monitor.py:
...
def detect_manual_polling(command: str) -> tuple[bool, str | None]:
    ...
    like ci_monitor.py. This is a code smell that should trigger investigation.
...
def is_command_with_comment_content(command: str) -> bool:
    ...
    - gh pr comment --body "This uses ci_monitor.py instead of gh pr checks"
    ...
    try:
        ...
        if re.search(r"gh\s+pr\s+checks\s+.*--watch", command):
            ...
            reason = (
                ...
                f"ci_monitor.py を使用してください:\n\n"
                f'python3 "$CLAUDE_PROJECT_DIR"/.claude/scripts/ci_monitor.py {pr_display} '
                f"--session-id <SESSION_ID>\n\n"
                f"※ <SESSION_ID>はUserPromptSubmit hookで提供されるセッションID\n\n"
                f"ci_monitor.py は以下を自動処理します:\n"
                f"  - BEHIND検知→自動リベース\n"
                f"  - レビュー完了検知→コメント取得\n"
                f"  - CI失敗→即座に通知"
            )
        ...
        # Block: gh run watch (verbose output, use ci_monitor.py instead)
        ...
        if re.search(r"gh\s+run\s+watch\b", strip_quoted_content(command)):
            reason = (
                ...
                "【PR関連のCI監視の場合】\n"
                "ci_monitor.py を使用してください:\n"
                '  python3 "$CLAUDE_PROJECT_DIR"/.claude/scripts/ci_monitor.py {PR番号} '
