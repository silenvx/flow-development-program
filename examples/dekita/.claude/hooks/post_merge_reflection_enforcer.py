...
def save_immediate_pending_state(session_id: str, action: str, context: str) -> None:
    ...
    Args:
        ...
        action: The action to execute (e.g., "/reflect").
        context: Context for the action (e.g., "PR #123 merge").
    """
    try:
...
def main():
    ...
    Issue #2690: Save pending state for early detection by
    immediate-pending-check.py (UserPromptSubmit hook).
    """
    ...
    try:
        ...
        if is_pr_merge_command(command) and _check_merge_success(tool_result, command):
            ...
            if session_id:
                save_immediate_pending_state(
                    session_id,
                    action="/reflect",
                    context=f"PR #{pr_number or '?'} merge",
                )
