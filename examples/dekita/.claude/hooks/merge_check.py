...
def main():
    ...
    if args.dry_run:
        if not args.pr_number:
            ...
            print("Usage: merge_check.py --dry-run <pr_number>", file=sys.stderr)
            sys.exit(2)
    ...
    try:
        ...
        if contains_rest_api_merge(command):
            ...
            log_hook_execution("merge_check", "block", "REST API merge blocked")
            result = make_block_result("merge_check", reason)
            print(json.dumps(result))
            sys.exit(0)

        # Not a merge command, log and skip silently (no output per design principle)
        if not is_merge_command:
            log_hook_execution("merge_check", "skip", "Not a merge command")
            sys.exit(0)
        ...
        if is_merge_command and has_auto:
            ...
            log_hook_execution("merge_check", "block", "--auto option blocked")
            result = make_block_result("merge_check", reason)
            print(json.dumps(result))
            sys.exit(0)
        ...
        if is_merge_command and has_admin:
            ...
            reason = "\n".join(reason_parts)
            log_hook_execution("merge_check", "block", "--admin option blocked")
            result = make_block_result("merge_check", reason)
            print(json.dumps(result))
            sys.exit(0)
        ...
        for warning in all_warnings:
            print(warning, file=sys.stderr)
            log_hook_execution("merge_check", "warning", warning)
        ...
        if blocking_reasons:
            ...
            result = make_block_result("merge_check", combined_reason)
        ...
        log_hook_execution("merge_check", "approve", "All checks passed")
        sys.exit(0)

    except Exception as e:
        # On error, approve to avoid blocking
        error_msg = f"Hook error: {e}"
        print(f"[merge_check] {error_msg}", file=sys.stderr)
        result = make_approve_result("merge_check", error_msg)
        # Log the error explicitly since make_approve_result doesn't set "reason"
        log_hook_execution("merge_check", "approve", error_msg)
        print(json.dumps(result))
        sys.exit(0)
