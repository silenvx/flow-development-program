...
description: 新機能調査、提案、設計方針確認時に使用。
---
...
| 機能 | 使用状況 |
|------|----------|
| Skills | `development-workflow`, `code-review`, `reflection-guide`, `coding-standards`, `hooks-reference` |
| Hooks | 多数設定済み（詳細は `hooks-reference` Skill参照） |
| Slash Commands | `/reflect`（振り返り実行） |
| Subagents | 未使用 |
...
- `/reflect`: 振り返りの明示的実行（意図的なタイミングで使用するため例外）
