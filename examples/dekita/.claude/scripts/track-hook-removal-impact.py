...
def analyze_removal_impact(removal: dict, issues: list[dict], commits: list[dict]) -> dict:
    ...
    # Also use the hook's purpose as a search term
    metadata = load_json(METADATA_PATH)
    hook_meta = metadata.get("hooks", {}).get(hook_name, {})
    purpose = hook_meta.get("purpose", "")
    if purpose:
        # Extract key terms from purpose
        purpose_words = re.findall(r"[\w]+", purpose)
        keywords.extend([w for w in purpose_words if len(w) > 3])
