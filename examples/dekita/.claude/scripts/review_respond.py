...
# Import record_response from record_review_response.py (Issue #1432)
# The file uses dynamic import to handle sys.path changes
_SCRIPT_DIR = Path(__file__).parent
_record_response_path = _SCRIPT_DIR / "record_review_response.py"
...
# record_review_response.py のモジュールレベルで sys.path が変更されるため、
    ...
    if len(sys.argv) == 3 and sys.argv[2] == "--list":
        ...
        for t in threads:
            ...
            if comments:
                ...
                print(f"  Body: {body}...")
                print(
                    f'  Command: python3 .claude/scripts/review_respond.py {pr_number} {comment_id} {thread_id} "<message>"'
                )
                print()
        return
