#!/bin/bash
# GeminiとCodexでIssueをレビューし結果をコメント投稿する。
#
# Why:
#     AIによるIssueレビューを自動化し、
#     問題の明確さや技術的実現性を事前に検証するため。
#
# What:
#     - run_gemini_review(): Gemini CLIでレビュー実行
#     - run_codex_review(): Codex CLIでレビュー実行
...
#
# Changelog:
#     - silenvx/dekita#1600: IssueのAIレビュー機能を追加
...
以下のGitHub Issueをレビューしてください。

レビュー観点:
1. 問題の明確さ: 問題が具体的に説明されているか
2. 再現手順: 問題を再現するための情報が十分か
3. 期待動作: 期待される動作が明確か
4. 技術的実現性: 提案された解決策は妥当か
5. 優先度の妥当性: ラベルや説明から優先度は適切か
...
run_gemini_review() {
    ...
    local output
    if output=$(echo "$REVIEW_PROMPT" | gemini -p "あなたはGitHub Issueレビューアーです。簡潔に日本語でレビューしてください。" 2>/dev/null); then
        echo "$output"
    else
        echo "$GEMINI_FAILED"
    fi
...
run_codex_review() {
    ...
    local codex_prompt="あなたはGitHub Issueレビューアーです。簡潔に日本語でレビューしてください。
